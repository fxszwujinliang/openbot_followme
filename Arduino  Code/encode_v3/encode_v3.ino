...
// PIN_PWM_L1,PIN_LEN,PIN_PWM_R1,PIN_REN      Low-level control of left DC motors via PWM 
...
//Setup the pin definitions
  ...
  #define PIN_ECHO 13 //4
  #define PIN_LED_LB 10 //7
  #define PIN_LED_RB 11 //8
#elif (OPENBOT == PCB_V1)
  #define PIN_PWM_L1 9
  #define PIN_LEN 10
  #define PIN_PWM_R1 5
...
void update_left_motors() {
    if (ctrl_left < 0) {
      ...
      analogWrite(PIN_LEN,LOW);
    }
    else if (ctrl_left > 0) {
      analogWrite(PIN_PWM_L1,ctrl_left);
      analogWrite(PIN_LEN,HIGH);
    }
    else { //Motor brake
...
void update_right_motors() {
    if (ctrl_right < 0) {
      analogWrite(PIN_PWM_R1,ctrl_left);
      analogWrite(PIN_REN,LOW);
    }
    else if (ctrl_right > 0) {
      analogWrite(PIN_PWM_R1,ctrl_right);
      analogWrite(PIN_REN,HIGH);
    }
    else { //Motor brake
