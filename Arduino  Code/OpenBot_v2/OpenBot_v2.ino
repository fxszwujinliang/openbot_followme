...
#if SWITCH
  bool flag=1;
  #define PIN_SWITCH_D0 A0
  #define PIN_SWITCH_D1 A1
  #define PIN_SWITCH_D2 A2
  #define PIN_SWITCH_D3 A3
#endif
...
void setup()
{
  ...
  pinMode(PIN_SWITCH_D0,INPUT);
  pinMode(PIN_SWITCH_D1,INPUT);
  pinMode(PIN_SWITCH_D2,INPUT);
  pinMode(PIN_SWITCH_D3,INPUT);
...
void loop() {
  #if SWITCH
    if(flag==1){
      while(digitalRead(PIN_SWITCH_D0)==0){//如果开关不启动，则不执行下方功能
//        Serial.println("Wait for switch!");delay(1);
      }
      flag=0;//结束该功能
      pinMode(PIN_SWITCH_D0,1);
      for(int i=0;i<5;i++){
        digitalWrite(PIN_SWITCH_D0,1);delay(500);
        digitalWrite(PIN_SWITCH_D0,0);delay(500);
      }
    }
    if(digitalRead(PIN_SWITCH_D1)==1){
      analogWrite(PIN_PWM_L1,200);
      analogWrite(PIN_PWM_L2,0);
      analogWrite(PIN_PWM_R1,0);
      analogWrite(PIN_PWM_R2,200);
    ...
    if(digitalRead(PIN_SWITCH_D2)==1){
      analogWrite(PIN_PWM_L1,0);
      analogWrite(PIN_PWM_L2,200);
    ...
    if(digitalRead(PIN_SWITCH_D3)==1){
      analogWrite(PIN_PWM_L1,200);
      analogWrite(PIN_PWM_L2,0);
      analogWrite(PIN_PWM_R1,200);
      analogWrite(PIN_PWM_R2,0);
