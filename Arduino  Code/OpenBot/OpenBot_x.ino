#include <Encoder.h>     //IDE安装Encoder库之后使用

// ---------------------------------------------------------------------------
// This Arduino sketch accompanies the OpenBot Android application. 
// By Matthias Mueller, Intelligent Systems Lab, 2021年10月
// 追加了Encoder测速模块
...
#define OPENBOT DIY   //确保程序进入"#if (OPENBOT == DIY)"分支 

// Enable/Disable voltage divider (1,0)
#define HAS_VOLTAGE_DIVIDER 0  //电池分压回路

// Enable/Disable indicators (1,0)
#define HAS_INDICATORS 0   //显示模块

// Enable/Disable speed sensors (1,0) 速度检测
#define HAS_SPEED_SENSORS 0 //我用Encoder和本程序自己手动检测的方法不同,这里定义为0 

// Enable/Disable sonar (1,0)
#define HAS_SONAR 1 //声呐超声波模块,没有这个模块好像电机一直全速运转

// Enable/Disable median filter for sonar measurements (1,0)
#define USE_MEDIAN 1  //声呐模块用到滤波算法，这里只是从众多数据中提取"中间值"

// Enable/Disable OLED (1,0) 
#define HAS_OLED 0   //OLED显示器

// Enable/Disable no phone mode (1,0)
// In no phone mode:    不使用手机,小车运动不受手机控制
...
//Setup the pin definitions  引脚定义
#if (OPENBOT == DIY)   //我的模式：根据上面宏定义,这里括号内表达式肯定为true，
  #define PIN_PWM_L1 11  //左电机1的PWM引脚
  #define PIN_PWM_L2 6   //左电机的转向控制引脚
  #define PIN_PWM_R1 3   //右电机的PWM引脚
  #define PIN_PWM_R2 10  //右电机的转向控制引脚
  //#define PIN_SPEED_L 2  //左速度检测引脚，我使用Encoder库，实际在创建Encoder对象的构造函数中设置，这里不需要设置
  //#define PIN_SPEED_R 3
  #define PIN_VIN A7   //分压的输入口
...
const unsigned int STOP_THRESHOLD = 12; //cm
  ...
  #include <PinChangeInterrupt.h>    //需要安装专用的库
#endif
...
int ctrl_left = 0;
int ctrl_right = 0;
//encoder measurement 编码器相关的变量定义
int counter_left = 0;
int counter_right = 0;
int oldstate_left = 0;
...
//用Encoder类实例化两个对象,其构造函数中传参是Encoder的两个引脚号。其他成员函数等参见class源码
Encoder LeftEnc(2, 4); //左Encoder的Ain和Bin分别为2和4引脚
...
//Speed sensor 这个是用码盘自己检测速度所用的变量，与我自带Encoder不同
...
void setup()
{
  ...
  #if HAS_SPEED_SENSORS   //上面把这个宏定义设置为0了.
  ...
  #if HAS_INDICATORS   //这些宏定义都被设置为0了
...
void loop() {
    ...
    if (millis() >= ping_timeout) {
      ...
      #endif
      send_ping();  //超声波测距，不用库，相当于重写，还用了模拟中断，嗯，
    }
  #endif
  ...
  #else // Wait for messages from the phone 这里正式接收手机传下来的信息
  ...
  //Measure speed, otherwise through interrupt
   update_speed_left();   //非中断模式？
   update_speed_right();

   // Send vehicle measurments to serial every SEND_INTERVAL定时发送小车速度信息
  if (millis() >= send_timeout) {
    send_vehicle_data();
    send_timeout = millis() + SEND_INTERVAL;
  }

  // Check indicator signal every INDICATOR_INTERVAL这个模块没启用，可以注释掉这段代码的
  //if (millis() >= indicator_timeout) {
    //update_indicator();
    //indicator_timeout = millis() + INDICATOR_INTERVAL;
  //}
